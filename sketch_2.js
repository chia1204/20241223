...
function displayControls() {
  ...
  // Player 1 操控提示
  text("Player 1 Controls:", 50, height - 80);
  text("Move Left: A", 50, height - 60);
  text("Move Right: D", 50, height - 40);
  text("Jump: W", 50, height - 20);
  text("Throw Bomb: F", 50, height, 100);
  
  // Player 2 操控提示
  text("Player 2 Controls:", width - 250, height - 80);
  text("Move Left: Left Arrow", width - 250, height - 60);
  text("Move Right: Right Arrow", width - 250, height - 40);
  text("Jump: Up Arrow", width - 250, height - 20);
  text("Throw Bomb: Space", width - 250, height, 100);
}

// 顯示淡江教科文字
function displayTamkangText() {
  fill(0); // 設定文字顏色為黑色
  textSize(80); // 設定文字大小
  stroke(10)
  // 顯示「淡江教科」文字
  text("淡江教科", 750 , 100);  // 在畫布中心顯示
}
